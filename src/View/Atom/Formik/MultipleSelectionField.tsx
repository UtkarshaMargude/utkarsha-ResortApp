        ...
        <Input
          ...
          startAdornment={
            <InputAdornment position="start">
              <IconButton>
                <SearchSharpMuiIcon sx={{ color: "primary.main" }} />
              </IconButton>
            </InputAdornment>
          }
        />
...
export function renderFilteredOptions(
  ...
  const filteredOptions =
    ...
    items.filter((item: IKeyValuePair | any) => {
      return (
        !item.value ||
        (item.value &&
          item.value.toString().toLowerCase().indexOf(query.toLowerCase()) !==
            -1)
      );
    });
  ...
  return filteredOptions?.length > 0
    ? filteredOptions.map(
        (option: { key: string | number; value: string | number }) => {
          return (
            <MenuItem
              key={option.key}
              value={option.key}
        ...
        renderValue={(selected) => {
          ...
          for (let i = 0; i < (selected as string[]).length; i++) {
            ...
            const value = items.find(
              (_ele: { key: string | number; value: string | number }) =>
                _ele.key === key
            )?.value;
            if (value) {
